...
def process(eid):
    df = pd.read_csv(os.path.join(dirpath, f"{eid}.csv"), index_col=0).reset_index(names='sample_id')
    df['eid'] = eid
    return df


dirpath: str = os.path.join(get_ae_path('validation'), 'simulation', 'state_paths')
...
for structure in eid_fpaths:
    ...
    if len(dfs) > 0:
        ...
        print('-----Saving output')
        output.to_csv(os.path.join(get_ae_path('validation'), 'simulation', 'state_paths', f'{structure}.csv'))
        del dfs
        del output
    else:
        print('-----No dfs.')
