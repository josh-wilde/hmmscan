...
sim_fits_s2c2_fpath <- file.path(validation_dirpath, 'simulation', 'fits', 's2c2.csv')
...
relevant_eids_sim_inputs <- sim_inputs %>% 
  select(eid, n_states, n_mix_comps, ovl, ovl_intra, mean_sojourn, input_s0_stat, n_lots_in_seqs) %>% 
  filter((n_states == 2) & (ovl == 0.25)) %>%
  filter((n_mix_comps == 1) | (ovl_intra == 0.50))

fits <- bind_rows(
  read_csv(sim_fits_s2c1_fpath, col_select = matches('^[a-z].*')),
  read_csv(sim_fits_s2c2_fpath, col_select = matches('^[a-z].*'))
) %>% 
...
hmm_accuracy %>% 
  ...
  ggplot(aes(x = n_lots_in_seqs, y = hmm_acc, color = factor(n_mix_comps))) + 
  geom_line(size=0.4) + 
