...
relevant_eids_sim_inputs <- sim_inputs %>%
  select(eid, n_states, n_mix_comps, threestate_id, ovl, ovl_intra, mean_sojourn, input_s2_stat, n_lots_in_seqs) %>% 
  filter((n_states == 3) & (threestate_id == 3))
...
hmm_accuracy %>% 
  ...
  facet_grid(rows = vars(mean_sojourn), cols = vars(factor(input_s2_stat))) + 
