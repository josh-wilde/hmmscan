...
sim_fits_s4_fpath <- file.path(validation_dirpath, 'simulation', 'fits', 's4.csv')
...
relevant_eids_sim_inputs <- sim_inputs %>%
  select(eid, n_states, n_mix_comps, threestate_id, ovl, mean_sojourn, n_lots_in_seqs) %>% 
  filter(n_states == 4)

fits <- read_csv(sim_fits_s4_fpath, col_select = matches('^[a-z].*')) %>% 
...
hmm_accuracy %>% 
  ...
  facet_grid(rows = vars(mean_sojourn)) + 
